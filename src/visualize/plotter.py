...
def plot_curves(list_y, legend=None, xy_labels=None, title=None, filename=None):
    """
    Plot curve, starting from x = 0
    y: list of curves: y = [c1, c2, ...]
    """
    line = []
    for i, it in enumerate(list_y):
        mark = np.random.choice(LINE_MARKERS)
        l = plt.plot(it, marker=mark, linewidth=1, markersize=4)
        line.append(l[0])
    if legend:
        tl = legend['title']
        leg = legend['legend']
        leg1 = plt.legend(line, leg, numpoints=1, loc='center right', title=tl)
        plt.gca().add_artist(leg1)
    if xy_labels:
        plt.xlabel(xy_labels[0], fontdict=plts.label_font)
        plt.ylabel(xy_labels[1], fontdict=plts.label_font)
    if title:
